...
- [x] Fundação visual + responsividade + conexão resiliente
- [x] Visão executiva (KPIs + evolução)
- [x] Pressão assistencial (ocupação SUS)
- [x] Views analíticas (mortalidade, UTI, hospitais)
- [x] Seção de hospitais
- [x] Filtros globais + per capita
- [x] Mapa de pontos (hospitais georreferenciados)
- [ ] **Choropleth por município/região** (mapa de calor por área — requer GeoJSON)
- [ ] Toggle Quantidade ↔ R$ na visão executiva
- [ ] Correlação de fatores associados à demanda

---

**Equipe DATAHOLICS** · FIAP Challenge — Parceria Oracle
